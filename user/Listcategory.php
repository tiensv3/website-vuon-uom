    ...
    <div class="row ">
        <div class="col-xl-3 col-lg-4 col-md-5 mt-5">
            <div class="sidebar-categories">
                <div class="head text-uppercase">Danh mục thương hiệu</div>
                <ul class="main-categories">
                    ...
                    while ($row = mysqli_fetch_array($result_select_cate)) {
                        // Check if the business has changed
                        if ($currentBusiness !== $row['businessname']) {
                            // If it has, close the previous business's categories and start a new one
                            if ($currentBusiness !== null) {
                                echo '</ul></li>';
                            }

                            echo '<li class="main-nav-list"><a data-toggle="collapse" href="#' . $row['businessname'] . '" aria-expanded="false" aria-controls="' . $row['businessname'] . '"><span class="lnr lnr-arrow-right"></span>' . $row['businessname'] . '</a>';
                            echo '<ul class="collapse" id="' . $row['businessname'] . '" data-toggle="collapse" aria-expanded="false" aria-controls="' . $row['businessname'] . '">';
                            // Update the current business
                            $currentBusiness = $row['businessname'];
                        }
                        // Display individual categories
                        echo '<li class="main-nav-list child"><a href="./Listcategory.php?action=danhmuc&id=' . $row['categoryid'] . '">' . $row['categoryname'] . '<span class="number">(XX)</span></a></li>';
                    }

                    // Close the last business's categories
                    if ($currentBusiness !== null) {
                        echo '</ul></li>';
                    }
                    ?>
            ...
            <div class="col-xl-9 col-lg-8 col-md-7 mt-5">
                ...
                <section class="lattest-product-area pb-40 category-list">
                    <div class="row">
                        ...
                        $sql_select_id_cate = "SELECT products.*, categories.categoryid 
                        FROM products INNER JOIN categories ON products.categoryid = categories.categoryid WHERE products.categoryid = '" . $id . "' LIMIT $limit OFFSET $offset";
                        $result_select_id_cate = $conn->query($sql_select_id_cate);

                        $total_pages_sql = "SELECT COUNT(*) FROM products WHERE products.categoryid = '" . $id . "'";
                        $total_pages_result = $conn->query($total_pages_sql);
                        $total_pages = ceil($total_pages_result->fetch_assoc()['COUNT(*)'] / $limit);
                        /* ---------------------------------- LOOP ---------------------------------- */
                        while ($product_All_id_cate = mysqli_fetch_array($result_select_id_cate)) {
                        ?>
                            <!-- Start hiển thị sản phẩm -->
                            <div class="col-lg-4 col-md-6">
                                <div class="single-product">
                                    <img class="" src="../<?php echo $product_All_id_cate['thumbnail'] ?>" alt="" width="300" height="250">
                                    <div class="product-details">
                                        <h6><?php echo $product_All_id_cate['productname'] ?></h6>
                                        <div class="price">
                                            <?php
                                            if ($product_All_id_cate['sale']) {
                                            ?>
                                                <h6><?php echo number_format($product_All_id_cate['sale']) . ' VNĐ' ?></h6>
                                            <?php
                                            }
                                            ?>
                                            <?php
                                            if (!$product_All_id_cate['sale']) {
                                            ?>
                                                <h6 class=""><?php echo number_format($product_All_id_cate['price']) . ' VNĐ' ?></h6>
                                            <?php
                                            } else {
                                            ?>
                                                <h6 class="l-through"><?php echo number_format($product_All_id_cate['price']) . ' VNĐ' ?></h6>
                                            <?php
                                            }
                                            ?>
                                        </div>
                                        <div class="prd-bottom">
                                            ...
                                            <div class="row">
                                                <div class="col-md-12">
                                                    ...
                                                    <a href="./DetailPro.php?action=chitiet&id=<?php echo $product_All_id_cate['productid'] ?>" class="social-info">
                ...
                <div class="filter-bar d-flex flex-wrap align-items-center">
                    ...
                    <div class="pagination">
                        <?php for ($i = 1; $i <= $total_pages; $i++) : ?>
                            <a href='./Listcategory.php?action=danhmuc&id=<?php echo $id ?>&page=<?php echo $i; ?>'><?php echo $i; ?></a>
                        <?php endfor; ?>
                    </div>
